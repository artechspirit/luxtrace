...
export default function TransactionDetailScreen() {
  ...
  const checkAndFetchQr = async (tx: Transaction) => {
    ...
    if (tx.type === 'PRIMARY_BOUTIQUE' && isOperator && tx.status === 'PENDING') {
      shouldFetchQr = true
    } else if (tx.type === 'P2P_DIRECT_HANDOVER' && !isBuyer && tx.status === 'PENDING') {
      shouldFetchQr = true
    } else if (tx.type === 'P2P_REMOTE_SHIPPING' && !isBuyer && (tx.status === 'PAID' || tx.status === 'IN_TRANSIT')) {
      shouldFetchQr = true
    }
    ...
    if (shouldFetchQr) {
      ...
      try {
        const response = await fetch(`${API_BASE_URL}/transactions/${tx.transaction_id}/qr`, {
          headers: {
            'Authorization': `Bearer ${token}`,
          },
        })
  ...
  const handleSimulatePayment = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/transactions/${id}/simulate-payment`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
        },
  ...
  if (errorMessage || !transaction) {
    return (
      <View style={styles.centerContainer}>
        ...
        <Text style={styles.errorText}>{errorMessage || 'Transaction not found'}</Text>
  ...
  } else if (transaction.status === 'CANCELLED' || transaction.status === 'FRAUD_FLAGGED') {
    statusColor = '#ef4444'
  ...
  return (
    <View style={[styles.container, { paddingTop: Math.max(insets.top, 8) }]}>
      ...
      <View style={styles.header}>
        ...
        <TouchableOpacity style={styles.backButton} onPress={fetchTransactionDetails}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        <View style={styles.statusCard}>
          ...
          <View style={styles.rowBetween}>
            <View>
              ...
              <Text style={styles.valueText}>{formatCurrency(transaction.amount_idr)}</Text>
            </View>
        ...
        <View style={styles.card}>
          ...
          <View style={styles.divider} />
          <Text style={styles.brandText}>{transaction.product?.brand.toUpperCase() || 'BRAND'}</Text>
          <Text style={styles.productName}>{transaction.product?.name || 'Loading Asset...'}</Text>
          
          <View style={styles.gridRow}>
            <View style={styles.gridCol}>
              <Text style={styles.metaLabel}>SERIAL NUMBER</Text>
              <Text style={styles.valueSubtext} numberOfLines={1}>{transaction.product?.serial_number}</Text>
            </View>
            <View style={styles.gridCol}>
              <Text style={styles.metaLabel}>NFT TOKEN ID</Text>
              <Text style={styles.valueSubtext}>
                {transaction.product?.nft_token_id ? `#${transaction.product.nft_token_id}` : 'MINT PENDING'}
              </Text>
            </View>
          </View>
        </View>
        ...
        {qrSession ? (
          <View style={styles.qrCard}>
            ...
            <Text style={styles.qrSubtext}>
              Display this code to the buyer. They must scan it using their Luxtrace app and tap the physical NFC chip to release ownership.
            </Text>
            ...
            <TouchableOpacity
              onPress={() => copyToClipboard(qrSession.session_id, 'Session ID')}
              style={styles.copyWrapper}
            >
              <Ionicons name="copy-outline" size={14} color="#00FFB2" />
              <Text style={styles.copyCodeText}>
                {qrSession.session_id.slice(0, 8)}...{qrSession.session_id.slice(-8)}
              </Text>
            </TouchableOpacity>
          ...
          <View style={styles.qrCard}>
            ...
            <Text style={styles.qrSubtext}>Generating Handover QR Session...</Text>
          </View>
        ) : null}

        {/* Buyer Actions / Waiting States */}
        {isBuyer && transaction.status === 'PENDING' && (transaction.type === 'P2P_REMOTE_SHIPPING' || transaction.type === 'PRIMARY_BOUTIQUE') && (
          <View style={styles.buyerActionCard}>
            <Ionicons name="cash-outline" size={32} color="#00FFB2" style={{ alignSelf: 'center', marginBottom: 12 }} />
            <Text style={styles.buyerActionTitle}>ESCROW DEPOSIT REQUIRED</Text>
            ...
            <TouchableOpacity
              style={transaction.payment_url ? styles.secondaryButton : styles.payButton}
              onPress={handleSimulatePayment}
              disabled={isSimulatingPayment}
            >
              {isSimulatingPayment ? (
                <ActivityIndicator color={transaction.payment_url ? '#00FFB2' : '#0A0A0A'} size="small" />
              ) : (
                <Text style={transaction.payment_url ? styles.secondaryButtonText : styles.payButtonText}>
                  {transaction.payment_url ? '🧪 SIMULATE ESCROW DEPOSIT' : 'SIMULATE ESCROW DEPOSIT'}
                </Text>
              )}
            </TouchableOpacity>
          </View>
        ...
        {isBuyer && (
          ...
          <View style={styles.buyerActionCard}>
            ...
            >
              <Text style={styles.payButtonText}>OPEN VERIFICATION SCANNER</Text>
            </TouchableOpacity>
          </View>
        )}
        ...
        <View style={styles.card}>
          ...
          <View style={styles.metaItemRow}>
            ...
            <TouchableOpacity onPress={() => copyToClipboard(transaction.transaction_id, 'Transaction ID')}>
              <Text style={styles.itemValCopy}>{transaction.transaction_id.slice(0, 18)}...</Text>
            </TouchableOpacity>
          </View>

          <View style={styles.metaItemRow}>
            <Text style={styles.itemLabel}>Transaction Type</Text>
            <Text style={styles.itemVal}>{transaction.type.replace('P2P_', '').replace('_', ' ')}</Text>
          </View>

          <View style={styles.metaItemRow}>
            <Text style={styles.itemLabel}>Created Date</Text>
            <Text style={styles.itemVal}>{formatDate(transaction.created_at)}</Text>
          </View>

          {transaction.completed_at && (
            <View style={styles.metaItemRow}>
              <Text style={styles.itemLabel}>Completed Date</Text>
              <Text style={styles.itemVal}>{formatDate(transaction.completed_at)}</Text>
            </View>
          )}
          ...
          {transaction.blockchain_tx_hash && (
            <View style={styles.metaItemRow}>
              ...
              <TouchableOpacity onPress={() => copyToClipboard(transaction.blockchain_tx_hash!, 'On-chain Tx Hash')}>
                <Text style={styles.itemValLink}>
                  {transaction.blockchain_tx_hash.slice(0, 10)}...{transaction.blockchain_tx_hash.slice(-8)}
                </Text>
              </TouchableOpacity>
            </View>
          )}
