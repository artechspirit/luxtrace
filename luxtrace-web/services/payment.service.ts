...
export const paymentService = {
  ...
  async createPayment(params: {
    ...
    if (!buyer) throw Object.assign(new Error('Buyer profile not found'), { code: 'NOT_FOUND' })
    ...
    if (type === 'PRIMARY_BOUTIQUE') {
      if (product.status !== 'REGISTERED') {
        throw Object.assign(new Error('Product is not available for purchase'), {
          code: 'PRODUCT_NOT_AVAILABLE',
        })
      }
    } else if (type === 'P2P_REMOTE_SHIPPING') {
      if (product.status !== 'OWNED') {
        throw Object.assign(new Error('Product is not available for P2P sale'), {
          code: 'PRODUCT_NOT_AVAILABLE',
        })
      }
      if (product.current_owner_id !== sellerId) {
        throw Object.assign(new Error('Caller is not the product owner'), { code: 'NOT_OWNER' })
      }
    }
    ...
    try {
      ...
      // Roll back transaction record if Midtrans fails
      await transactionRepository.updateStatus(transaction.transaction_id, 'CANCELLED')
      throw Object.assign(new Error('Payment gateway error'), { code: 'PAYMENT_INIT_FAILED', cause: err })
    }

    // 7. Persist payment_ref for idempotent webhook lookup
    await transactionRepository.setPaymentRef(transaction.transaction_id, orderId)
  ...
  async processWebhook(payload: MidtransWebhookPayload): Promise<void> {
    ...
    const isValid = validateMidtransSignature(order_id, status_code, gross_amount, signature_key)
    if (!isValid) {
      throw Object.assign(
    ...
    if (
      ...
      console.log(`[Webhook] Order ${order_id} already in terminal state: ${transaction.status}`)
      return
    }
    ...
    if (fraud_status === 'challenge') {
      ...
      await transactionRepository.updateStatus(transaction.transaction_id, 'FRAUD_FLAGGED')
      return
    }

    if (transaction_status === 'settlement' || transaction_status === 'capture') {
      await paymentService._handleSettlement(transaction.transaction_id, order_id)
      return
    }
    ...
    console.log(`[Webhook] No action for status: ${transaction_status}, order: ${order_id}`)
  },
   ...
   */
  async _handleSettlement(transactionId: string, orderId: string): Promise<void> {
    // Mark PAID
    await transactionRepository.updateStatus(transactionId, 'PAID')

    const transaction = await transactionRepository.findById(transactionId)
    if (!transaction) throw new Error('Transaction disappeared after PAID update')
    ...
    if (!product || !buyer) throw new Error('Data integrity error: missing product or buyer')

    const brandWallet = process.env.BRAND_WALLET_ADDRESS!
    if (!product.nft_token_id) throw new Error(`Product ${transaction.product_id} has no NFT token`)
    ...
    try {
      ...
      await paymentService._refundOrder(orderId, transaction.amount_idr, 'NFT transfer failed')
      await transactionRepository.updateStatus(transactionId, 'CANCELLED')
      throw err
    }
   ...
   */
  async _refundOrder(orderId: string, amountIdr: number, reason: string): Promise<void> {
    try {
  ...
  async syncTransactionStatus(transactionId: string): Promise<void> {
    ...
    const midtransStatus = await getMidtransTransactionStatus(transaction.payment_ref)
    ...
    if (
      ...
      await paymentService._handleSettlement(transactionId, transaction.payment_ref)
    } else if (
